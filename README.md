...
| [iFlytek SkillHub](https://github.com/iflytek/skillhub) | Self-hosted, open-source agent skill registry for enterprises by iFlytek (科大讯飞). Publish & version skill packages, RBAC governance with audit logs, deploy on-premise with Docker or Kubernetes. |
...
| [feishu-cli](https://github.com/riba2534/feishu-cli) | Full-featured Feishu (Lark) CLI — docs, wiki, sheets, messages, calendar, tasks. **11 SKILL.md files** for AI agents. Lossless Markdown ↔ Feishu doc conversion (40+ block types). 10K+ line concurrent pipeline. |
| [WeCom CLI](https://github.com/WecomTeam/wecom-cli) | **WeCom (企业微信) official CLI** — contacts, tasks, meetings, messages, schedules, docs, and smart forms. 7 categories, 12 AI Agent Skills. `npm i -g @wecom/cli`. Designed for both humans and AI agents. |
| [DingTalk Workspace CLI](https://github.com/DingTalk-Real-AI/dingtalk-workspace-cli) | **DingTalk's official CLI** — cross-platform, designed for both humans and AI agents. Covers contacts, calendar, tasks, attendance, AI tables, group chat, and logs. 12 Python skill scripts, three-layer SKILL.md architecture, MCP protocol. AES-256-GCM encrypted auth. |
| [Alibaba Cloud Data Agent Skill](https://github.com/aliyun/data-agent-skill) | Alibaba Cloud's official agent skill for data analysis. Natural language queries to DMS databases, CSV/Excel/JSON file analysis, async execution with session isolation. |
...
| [7 Principles for Agent-Friendly CLIs](https://x.com/trevin/status/2037250000821059933) | Trevin (Compound Engineering) | The missing CLI design rubric for agents: non-interactive by default, structured output, actionable errors, safe retries, progressive help, composable structure, bounded responses. Severity-based (Blocker/Friction/Optimization), not pass/fail. |
